                ...
                "Commands": [
                    {
                        "SemanticQueryDataShapeCommand": {
                                ...
                                "Where": [
                                    {
                                        "Condition": {
                                            "In": {
                                                ...
                                                "Values": [[{"Literal": {"Value": "'DUMMY_YEAR'"}}]]
                                            }
                                        }
                                    },
                                    {
                                        "Condition": {
                                            "In": {
                                                "Expressions": [
                                                    {"Column": {"Expression": {"SourceRef": {"Source": "s"}}, "Property": "StateName"}}
                                                ],
                                                "Values": [[{"Literal": {"Value": "'DUMMY_STATE'"}}]]
                                            }
                                        }
                                    }
                                ],
                            ...
                            "Binding": {
                                "Primary": {
                                    "Groupings": [
                                        {"Projections": list(range(14)), "Subtotal": 1}
                                    ]
                                },
                                "DataReduction": {
                                    "DataVolume": 3,
...
def fetch_data(year, state):
    current_payload = copy.deepcopy(payload_template)
    current_payload["queries"][0]["Query"]["Commands"][0]["SemanticQueryDataShapeCommand"]["Query"]["Where"][0]["Condition"]["In"]["Values"][0][0]["Literal"]["Value"] = f"'{year}'"
    current_payload["queries"][0]["Query"]["Commands"][0]["SemanticQueryDataShapeCommand"]["Query"]["Where"][1]["Condition"]["In"]["Values"][0][0]["Literal"]["Value"] = f"'{state}'"
        ...
        while True:
            ...
            if output_path.exists():
                with open(output_path) as f:
                    data = json.load(f)
                    rt = data.get("results")[0].get("result", {}).get("data", {}).get("dsr", {}).get("DS")[0].get("RT")
                    if rt:
                        current_payload["queries"][0]["Query"]["Commands"][0]["SemanticQueryDataShapeCommand"]["Binding"]["DataReduction"]["Primary"]["Window"]["RestartTokens"] = rt
                        i += 1
                        continue
            response = requests.post(url, headers=headers, json=current_payload, timeout=120)
